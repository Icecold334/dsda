        ...
        <table class="w-full border-3 border-separate border-spacing-y-4 h-5">
            ...
            <tbody>
                ...
                <tr class="bg-gray-50 hover:bg-gray-200 hover:shadow-lg transition duration-200 rounded-2xl">
                    ...
                    <!-- Jenis KDO -->
                    <td class="py-3 px-6">
                        <input type="text" disabledwire:model.live.debounce.500ms="list.{{ $index }}.aset_name"
                    ...
                    <!-- Deskripsi Kerusakan -->
                    <td class="px-6 py-3">
                        <textarea id="deskripsiKerusakan"
                            disabledwire:model.live.debounce.500ms="list.{{ $index }}.deskripsi" rows="2"
                    ...
                    <!-- Catatan -->
                    <td class="px-6 py-3">
                        <textarea id="catatan" disabledwire:model.live.debounce.500ms="list.{{ $index }}.catatan"
                            rows="2"
                            class="w-full border cursor-not-allowed border-gray-300 rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500"
                    ...
                    <td class="py-3 px-6">
                        ...
                        <selectwire:model.live.debounce.500ms="list.{{ $index }}.aset_id" disabled
                            ...
                            @if (is_null($list[$index]['aset_id']))
                            <div class="mt-2">
                                <input type="text" wire:model.live.debounce.500ms="list.{{ $index }}.noseri" disabled
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5"
                                    placeholder="No. Plat">

                                <input type="text" wire:model.live.debounce.500ms="list.{{ $index }}.jenis_kdo" disabled
                                ...
                                <input type="text" wire:model.live.debounce.500ms="list.{{ $index }}.nama_kdo" disabled
                                    class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5"
                    ...
                    <td class="px-6 py-3">
                        <textarea id="namadriver" disabledwire:model.live.debounce.500ms="list.{{ $index }}.driver_name"
                            rows="1"
                            class="w-full border cursor-not-allowed border-gray-300 rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500"
                            placeholder="Masukan Nama Driver"></textarea>
                    </td>
                    {{-- <td class="py-3 px-6">
                        <selectwire:model.live.debounce.500ms="list.{{ $index }}.driver_id" disabled
                            class="bg-gray-50 border border-gray-300   text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                    ...
                    @if ($kategori_id != 6)
                    <td class="py-3 px-6">
                        <selectwire:model.live.debounce.500ms="list.{{ $index }}.barang_id" disabled
                    ...
                    <td class="py-3 px-6">
                        <div class="flex items-center">
                            <input type="number" wire:model.live.debounce.500ms="list.{{ $index }}.jumlah" min="1"
                                disabled
                    ...
                    <td class="py-3 px-6">
                        <div class="flex items-center">
                            <input type="number" wire:model.live.debounce.500ms="list.{{ $index }}.jumlah_approve"
                                min="1" disabled
                    ...
                    <td class="px-6 py-3 text-center">
                        <div class="relative inline-block">
                            ...
                            <div class="relative inline-block">
                                <a href="{{ asset('storage/kondisiKdo/' . $item['img_done']) }}" target="_blank">
                            ...
                            <div class="relative inline-block">
                                <a href="{{ $item['img_done']->temporaryUrl() }}" target="_blank">
                                    <img src="{{ $item['img_done']->temporaryUrl() }}" alt="Preview Bukti"
                                        class="w-16 h-16 rounded-md">
                                </a>
                            ...
                            <input type="file" wire:model.live.debounce.500ms="list.{{ $index }}.img_done"
                                class="hidden" id="upload-list.{{ $index }}.img_done">
                            @php
                            $isPenjagaGudang = auth()->user()?->hasRole('Penjaga Gudang');
                            $isStatusAktif = $item['detail_permintaan_status']; // pastikan $status didefinisikan
                            sebelumnya
                            ...
                            <button type="button"
                                ...
                                class="text-primary-700 bg-gray-200 border text-sm border-primary-500 rounded-lg px-3 py-1.5 transition
                                                    {{ !$isEnabled ? 'opacity-50 cursor-not-allowed pointer-events-none' : 'hover:bg-primary-600 hover:text-white' }}" {{ !$isEnabled ? 'disabled'
                                : '' }}>
                                Unggah Foto
                            </button>
                            @endif
                    ...
                    <td class="px-6 py-3 text-center">
                        <div class="relative inline-block">
                            ...
                            @endphp
                            <button @if ($canUpload)
                                onclick="document.getElementById('upload-list.{{ $index }}.img_done').click()" @endif
                                class="text-primary-700 bg-gray-200 border text-sm border-primary-500 rounded-lg px-3 py-1.5 transition
                    ...
                    <td class="px-6 py-3 text-center">
                        <div class="relative inline-block">
                            ...
                            <input type="text" wire:model.live.debounce.500ms="list.{{ $index }}.voucher_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 focus:ring-blue-500 focus:border-blue-500
                                                {{ $isDisabled ? 'cursor-not-allowed bg-gray-200' : '' }}"
                                placeholder="Nama Voucher" @if ($isDisabled) disabled @endif>
                            @endcan
                        </div>
                    </td>
                    @endif
                    ...
                    <td class="py-3 px-6 text-center {{ $kategori_id != 4 ? '' : 'hidden' }}">
                        <div class="flex gap-2 justify-center">
                            ...
                            <button wire:click="openApprovalModal({{ $item['id'] }})"
                                ...
                                {{ $item['detail_permintaan_status'] && $item['detail_permintaan_cancel']==0
                                ? 'disabled' : '' }}>
                                Detail
                            </button>
                            @endif
                            ...
                            @if (is_null($status) && auth()->id() !== $item['user_id'] &&
                            ...
                            <span onclick="viewNoteFixKDO(`{{ addslashes($item['catatan'] ?? '') }}`)"
                ...
                @if (
                $showAdd &&
                (($kategori_id === 6 && count($list) < 1) || ($kategori_id===5 && count($list) < 2) || ($kategori_id
                    !==6 && $kategori_id !==5))) <tr
                    ...
                    <td class="py-3 px-6 relative">
                        <input type="text" wire:model.live.debounce.500ms="newAset" wire:focus="focusAset"
                    ...
                    <!-- Deskripsi Kerusakan -->
                    <td class="px-6 py-3">
                        <textarea id="deskripsiKerusakan" wire:model.live.debounce.500ms="newDeskripsi" rows="2"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500"
                            placeholder="Deskripsikan kerusakan secara detail"></textarea>
                    </td>
                    <!-- Catatan -->
                    <td class="px-6 py-3">
                        <textarea id="catatan" wire:model.live.debounce.500ms="newCatatan" rows="2"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500"
                            placeholder="Tambahkan catatan jika diperlukan"></textarea>
                    </td>
                    @endif
                    ...
                    <!-- Jenis KDO -->
                    {{-- <td class="py-3 px-6 relative">
                        <input type="text" wire:model.live.debounce.500ms="newLokasi" wire:focus="focusLokasi"
                    ...
                    <td class="py-3 px-6">
                        <selectwire:model.live.debounce.500ms="newAsetId"
                            ...
                            @if ($newAsetId === '0')
                            <div class="mt-2">
                                <input type="text" wire:model.live.debounce.500ms="NoSeri"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5"
                                    placeholder="No. Plat">

                                <input type="text" wire:model.live.debounce.500ms="JenisKDO"
                                    class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5"
                                    placeholder="Jenis Mobil">

                                <input type="text" wire:model.live.debounce.500ms="NamaKDO"
                                    class="mt-2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5"
                                    placeholder="Merk / Nama Mobil">
                            </div>
                            @endif
                    ...
                    <td class="px-6 py-3">
                        <textarea id="namadriver" wire:model.live.debounce.500ms="newDriverId" rows="1"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-primary-500 focus:border-primary-500"
                            placeholder="Masukan Nama Driver"></textarea>
                    </td>
                    {{-- <td class="py-3 px-6">
                        <selectwire:model.live.debounce.500ms="newDriverId"
                            class="bg-gray-50 border border-gray-300   text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="">Pilih Driver
                            </option>
                            @foreach ($drivers as $driver)
                    ...
                    <td class="py-3 px-6">
                        <selectwire:model.live.debounce.500ms="newBarangId" wire:change="selectMerk"
                            class="block w-full px-4 py-2 text-gray-900 border border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500">
                    ...
                    <td class="px-6 py-3 text-center">
                        ...
                        @else
                        <input type="file" wire:model.live.debounce.500ms="newBukti" class="hidden"
                            id="upload-newBukti">
