    ...
    <table class="w-full border-separate border-spacing-y-4">
        <thead>
            <tr class="text-white">
                ...
                @endif
                <th class="py-3 px-6 bg-primary-950 text-center font-semibold w-1/5" @if ($showDokumen && auth()->
                    user()->unitKerja->hak)
                    colspan="2" @endif>JUMLAH *</th>
                @if ($showDokumen)
                <th class="py-3 px-6 bg-primary-950 text-center font-semibold">DOKUMEN PENDUKUNG</th>
                <th class="py-3 px-6 bg-primary-950 text-center {{ $showDokumen ? 'rounded-r-lg' : '' }} font-semibold">
                </th>
                @else
        ...
        <tbody>
            @if ($showDokumen && auth()->user()->unitKerja->hak)
            <tr>
                <th colspan="5"></th>
                <th>Diajukan *</th>
                <th>Diterima </th>
            ...
            <tr class="bg-gray-50 hover:bg-gray-200 hover:shadow-lg transition duration-200 rounded-2xl">
                ...
                <td class="px-2 py-3">
                    <div class="flex items-center">
                        <input type="number" {{-- wire:model.fill="list.{{ $index }}.jumlah" --}}
                            value="{{ $item['jumlah'] }}" wire:input="updateJumlah({{ $index }}, $event.target.value)"
                        ...
                        <span
                            ...
                            {{ $item['merk_id'] ?
                            optional(App\Models\MerkStok::find($item['merk_id'])->barangStok->satuanBesar)->nama :
                            'Satuan' }}
                        </span>
                    ...
                    <div class="relative inline-block">
                        <a href="{{ is_string($item['bukti'])
                            ...
                            <img src="{{ is_string($item['bukti'])
                                            ...
                                            : (is_object($item['bukti']) && method_exists($item['bukti'], 'temporaryUrl')
                                                ? $item['bukti']->temporaryUrl()
                                                : null) }}" alt="Bukti" class="w-16 h-16 rounded-md">
                        </a>
                        @if (
                        $item['editable'] &&
                ...
                <td class="text-center">
                    ...
                    @if ($showDokumen)
                    <button wire:click="updatePengirimanStok({{ $index }})" class="text-success-900 border-success-600 text-xl border bg-success-100 hover:bg-success-600 hover:text-white font-medium rounded-lg px-3 py-1 transition duration-200
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg w-2/3">
            ...
            <div class="flex justify-between items-center p-4 border-b">
                ...
                <button wire:click="openApprovalModal({{ $selectedItemId }})" class="text-gray-500 hover:text-gray-800">
                    &times;
                </button>
            </div>
            {{-- @dd($approvalData) --}}
                ...
                <table class="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="border px-4 py-2 {{ auth()->id() == $pengiriman['id'] ? 'font-semibold' : '' }}">
                                {{ $pengiriman['nama'] }}</td>

                            <td class="border px-4 py-2">
                                <input type="number" wire:model="approvalData.{{ $index }}.jumlah"
                                    @disabled($pengiriman['jumlah'] || !$showButtonPemeriksa || auth()->id() !=
                                $pengiriman['id'])
                                class="w-full {{ $pengiriman['jumlah'] || !$showButtonPemeriksa || auth()->id() !=
                                $pengiriman['id'] ? 'cursor-not-allowed bg-gray-200' : '' }} border rounded px-2 py-1"
                                min="0">
                            </td>
                            <td class="border px-4 py-2">
                                <textarea wire:model="approvalData.{{ $index }}.catatan" @disabled($pengiriman['jumlah']
                                    || !$showButtonPemeriksa || auth()->id() != $pengiriman['id'])
            ...
            <div class="flex justify-end p-4 border-t">
                <button wire:click="approveItem({{ $selectedItemId }})" {{--
                    optional(collect($approvalData)->where('id', auth()->id())->first())['jumlah'] --}}
