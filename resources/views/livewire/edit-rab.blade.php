                ...
                <form wire:submit.prevent="save">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select wire:model.live="program_id" wire:loading.attr="disabled"
                        ...
                        <div>
                            ...
                            <select wire:model.live="kegiatan_id" wire:loading.attr="disabled"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                {{ !$canEdit ? 'disabled' : '' }}>
                        ...
                        <div>
                            ...
                            <select wire:model.live="sub_kegiatan_id" wire:loading.attr="disabled"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                {{ !$canEdit ? 'disabled' : '' }}>
                            ...
                            @error('sub_kegiatan_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Aktivitas Sub Kegiatan</label>
                            <select wire:model.live="aktivitas_sub_kegiatan_id" wire:loading.attr="disabled"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                {{ !$canEdit ? 'disabled' : '' }}>
                            ...
                            <div wire:loading wire:target="aktivitas_sub_kegiatan_id" class="text-sm text-blue-600 mt-1">
                                <i class="fas fa-spinner fa-spin"></i> Memuat data uraian rekening...
                            </div>
                            @error('aktivitas_sub_kegiatan_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Uraian Rekening</label>
                            <select wire:model.live="uraian_rekening_id" wire:loading.attr="disabled"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                {{ !$canEdit ? 'disabled' : '' }}>
                        ...
                        <div>
                            ...
                            <select wire:model.live="kecamatan_id" wire:loading.attr="disabled"
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                {{ !$canEdit ? 'disabled' : '' }}>
                        ...
                        <div>
                            ...
                            <textarea wire:model="keterangan_perubahan" {{ !$canEdit ? 'disabled' : '' }}
                                ...
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"></textarea>
                            @error('keterangan_perubahan') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($detailRabs as $detail)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $detail['nama_barang'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $detail['spesifikasi'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $detail['satuan'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $detail['kuantitas'] }}</td>
                                    @if($canEdit)
                ...
                <div class="mt-3">
                    ...
                    <form wire:submit.prevent="saveDetailRab" class="space-y-4">
                        <div>
                            ...
                            @error('selectedBarangId') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        @if(count($merkStoks) > 0)
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Pilih Merk/Spesifikasi</label>
                                <select wire:model.live="detailForm.merk_id"
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                                ...
                                @error('detailForm.merk_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                            </div>
                        @endif
                        ...
                        @if(!$detailForm['merk_id'])
                            <div class="border-t pt-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model="detailForm.nama_barang" 
                                        placeholder="Masukkan nama barang..."
                                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                                    @error('detailForm.nama_barang') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                                
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Spesifikasi</label>
                                    <textarea wire:model="detailForm.spesifikasi"
                                        placeholder="Masukkan spesifikasi barang..."
                                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500" rows="3"></textarea>
                                    @error('detailForm.spesifikasi') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model="detailForm.satuan"
                                        ...
                                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                                    @error('detailForm.satuan') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                </div>
                            </div>
                        @else
                            ...
                            <div class="bg-gray-50 p-3 rounded">
                                ...
                                <p class="text-sm text-gray-700"><strong>Spesifikasi:</strong> {{ $detailForm['spesifikasi'] }}</p>
                        ...
                        <div>
                            ...
                            <input type="number" wire:model="detailForm.kuantitas" step="0.01"
                                ...
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                            @error('detailForm.kuantitas') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea wire:model="detailForm.keterangan"
                                ...
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500" rows="3"></textarea>
                            @error('detailForm.keterangan') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
            ...
            <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-1/3 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea wire:model="reason_delete" 
                            placeholder="Jelaskan alasan penghapusan RAB..."
