<div>
    <table class="w-full border-separate border-spacing-y-4">
        <tr>
            ...
            <td>
                <div class="">
                    <input type="text" wire:model.live.debounce.500ms="query" wire:focus="focus" @disabled($listCount)
                        placeholder="Cari Vendor" wire:blur="hideSuggestions"
                        class="{{ $listCount ? 'cursor-not-allowed' : '' }} bg-gray-50 border border-gray-300  text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
        ...
        <tr class={{ !$cekSemuaItem ? 'hidden' : '' }}>
            <td class="w-1/3">
                <label for="nomor_kontrak" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nomor
                    Kontrak *</label>
            </td>
            <td>
        ...
        <tr class="{{ !$cekSemuaItem ? 'hidden' : '' }}">
            ...
            <td>
                <input type="date" wire:model.live.debounce.500ms="tanggal_kontrak"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
        ...
        <tr>
            ...
            <td>
                <selectwire:model.live.debounce.500ms="barang_id" @disabled($listCount) @disabled($vendor_id==null)
        ...
        <tr class="{{ !$cekSemuaItem ? 'hidden' : '' }}">
            ...
            <td>
                <selectwire:model.live.debounce.500ms="metode_id" @disabled($vendor_id==null)
    ...
    <div class="mt-3">
        <button wire:click="toggleAddVendorForm"
            class="text-sm text-white bg-primary-600 px-2 rounded-lg  hover:shadow-lg hover:bg-primary-400 transition duration-200">{{
            $showAddVendorForm ? 'Batal' : '+ Tambah Vendor Baru' }}</button>
    </div>
    @endif --}}
    ...
    <div class="mt-4 border p-4 rounded bg-gray-100">
        ...
        <div class="mb-3">
            ...
            <input type="text" wire:model.live.debounce.500ms="nama" class="border p-2 rounded w-full">
            @error('nama')
        ...
        <div class="mb-3">
            ...
            <input type="text" wire:model.live.debounce.500ms="alamat" class="border p-2 rounded w-full">
            @error('alamat')
            <span class="text-sm text-red-500">{{ $message }}</span>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="text" wire:model.live.debounce.500ms="kontak" class="border p-2 rounded w-full">
            @error('kontak')
            <span class="text-sm text-red-500">{{ $message }}</span>
            @enderror
        </div>
