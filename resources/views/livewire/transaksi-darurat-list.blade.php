        ...
        <tbody>
            ...
            <tr class="bg-gray-50 hover:bg-gray-200">
                ...
                <td class="px-2 py-3">
                    <div class="flex space-x-2">
                        ...
                        <input
                            ...
                            type="text" value="{{ $value }}" placeholder="{{ ucfirst($key) }}" disabled>
                        @endforeach
                    </div>
                </td>
                <td class="px-2 py-3">
                ...
                {{-- <td class="px-2 py-3">
                    ...
                    <selectwire:model.live.debounce.500ms='list.{{ $index }}.lokasi_id' disabled
                ...
                <td class="py-3 px-2">
                    <textarea
                        class="bg-gray-50 border cursor-not-allowed border-gray-300 text-gray-900 text-sm rounded-lg p-2.5 w-full"
                ...
                <td class="px-2 py-3">
                    <selectwire:model.live.debounce.500ms='list.{{ $index }}.ppn' @disabled(!0)
                ...
                <td class="px-2 py-3">
                    <div class="flex">
                        ...
                        <input
                            ...
                            wire:model.live.debounce.500ms='list.{{ $index }}.harga' @if (!0) disabled @endif>
                    </div>
                </td>

                <td class="text-center py-3 px-2">
                    <input type="file" wire:model.live.debounce.500ms="list.{{ $index }}.bukti" class="hidden"
                ...
                <td class="text-center py-3 px-1">
                    ...
                    <span
                        class="{{ $item['status'] ? 'text-warning-600' : ($item['status'] === 0 ? 'text-red-600' : 'text-gray-600') }}">
                        {{ is_null($item['status']) ? (empty($item['bukti']) ? 'Menunggu' : '') : ($item['status'] ?
                        'Disetujui' : 'Ditolak') }}
                    </span>
                    @endcan
                    @endif
            ...
            <tr>
                <td class="px-2 py-3">
                    <div class="flex space-x-2">
                        <input
                            ...
                            type="text" wire:model.live.debounce.500ms="newBarang" wire:focus='fillNewBarang'
                    ...
                    <div class="flex">
                        ...
                        <input @disabled(!$newBarangId)
                            class="bg-gray-50 border {{ !$newBarangId ? 'cursor-not-allowed' : '' }} border-gray-300 text-gray-900 text-sm 
                                    ...
                                    focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600" type="text"
                            wire:model.live.debounce.500ms="specifications.{{ $key }}"
                            wire:input="updateSpecification('{{ $key }}', $event.target.value)"
                            wire:focus="updateSpecification('{{ $key }}')" wire:blur="blurSpecification('{{ $key }}')"
                            placeholder="{{ $label }}">
                ...
                <td class="px-2 py-3">
                    <div class="flex">
                        <input @disabled(!($specifications['merek'] || $specifications['tipe'] ||
                            $specifications['ukuran']))
                            ...
                            type="number" wire:model.live.debounce.500ms="newJumlah" placeholder="Jumlah">
                        <div
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-r-lg focus:ring-blue-500 focus:border-blue-500 block max-w-full p-2.5 dark:bg-gray-700 dark:border-gray-600">
                ...
                <td class="py-3 px-2">
                    <textareawire:model.live.debounce.500ms="newLokasiPenerimaan"
                ...
                {{-- <td class="px-2 py-3">
                    <selectwire:model.live.debounce.500ms='newLokasiId'
                        class="
                            bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option value="">Pilih Lokasi</option>
                        @foreach ($lokasis as $lokasi)
                ...
                <td class="py-3 px-2">
                    <textareawire:model.live.debounce.500ms="newKeterangan"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5 w-full"
                ...
                <td class="px-2 py-3">
                    <div class="flex">
                        ...
                        <input id="newHarga" autocomplete="off"
                            ...
                            type="text" placeholder="Harga Satuan" oninput="formatRupiah(this)" value="{{ $newHarga }}"
                            @if (!$newJumlah) disabled @endif>
                ...
                <td class="py-3 px-2 text-center">
                    ...
                    <!-- Display uploaded proof preview with remove icon -->
                    <div class="relative inline-block">
                        <a href="{{ $newBukti->temporaryUrl() }}" download="{{ $newBukti->getClientOriginalName() }}">
                            <img src="{{ $newBukti->temporaryUrl() }}" alt="Bukti" class="w-16 h-16 rounded-md">
                        </a>
