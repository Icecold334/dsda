<div class="p-4">
    <h1 class="text-xl font-bold mb-4">Master Security</h1>
    ...
    <table class="w-full table-auto border">
        ...
        <tbody>
            ...
            <tr class="border-t">
                <td class="p-2">{{ $i + 1 }}</td>
                <td class="p-2">{{ $security->nama }}</td>
                <td class="p-2 space-x-2">
                    @can('security.update')
                    <button wire:click="openModalEdit({{ $security->id }})"
                        class="text-blue-600 hover:underline">Edit</button>
                    @endcan
                    @can('security.delete')
                    <button wire:click="delete({{ $security->id }})" class="text-red-600 hover:underline">Hapus</button>
                    @endcan
                </td>
            </tr>
            @endforeach
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white p-6 rounded-lg w-full max-w-md">
            <h2 class="text-lg font-semibold mb-4">{{ $isEdit ? 'Edit' : 'Tambah' }} Security</h2>
