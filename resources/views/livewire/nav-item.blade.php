    ...
    <li id="dropdown{{ $title }}Button" data-dropdown-toggle="dropdown{{ $title }}" data-dropdown-trigger="hover"
        ...
        <div id="dropdown{{ $title }}" class="z-10 hidden bg-white py-3  shadow-2xl max-w-max dark:bg-gray-700">
            <ul class=" font-normal text-sm text-gray-700 dark:text-gray-200 capitalize"
                ...
                @if ($item['title'] == 'Stok Sudin' && !auth()->user()->hasRole(['Admin Sudin','Kepala Suku Dinas']))
                @continue
                @endif
                <li>
                    <a href="{{ $item['href'] }}"
                        class="block px-3 py-4 -my-3  hover:bg-primary-950 transition duration-200 hover:text-white dark:hover:bg-gray-600 dark:hover:text-white">{{
                        $item['title'] }}</a>
                </li>
                @endforeach
            </ul>
