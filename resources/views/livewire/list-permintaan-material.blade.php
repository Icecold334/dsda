<div>
        ...
        <tbody>
            ...
            <tr class="bg-gray-50 hover:bg-gray-200 hover:shadow-lg transition duration-200 rounded-2xl">
                <td class="py-3 px-6 {{ $isSeribu && $withRab ? '' : 'hidden' }}">
                    <selectwire:model.live.debounce.500ms="list.{{ $index }}.rab_id" disabled class="bg-gray-50 border cursor-not-allowed border-gray-300 text-gray-900 text-sm rounded-lg
                ...
                <td class="py-3 px-6">
                    ...
                    {{-- @if(auth()->user()->hasRole('Pengurus Barang') && $permintaan->status == 1) --}}
                    <select wire:model.live.debounce.500ms="list.{{ $index }}.merk_id"
                ...
                <td class="py-3 px-6">
                    <div class="flex items-center">
                        ...
                        <input type="number" value="{{ $item['jumlah'] }}"
                            placeholder="Maksimal: {{ $stokTersedia ?? 0 }} (berdasarkan stok gudang)" disabled
                        ...
                        <span
                            ...
                            {{ isset($item['merk']) && isset($item['merk']->barangStok) ?
                ...
                <td class="px-6 py-3 {{ $isSeribu && $withRab ? '' : 'hidden' }}">
                    <textarea id="jumlah" wire:model.live.debounce.500ms="list.{{ $index }}.keterangan" disabled
                        rows="2"
            ...
            <tr
                ...
                <td class="py-3 px-6 {{ $isSeribu && $withRab ? '' : 'hidden' }}">
                    <selectwire:model.live.debounce.500ms="newRabId" class="bg-gray-50 border  border-gray-300 text-gray-900 text-sm rounded-lg
                                focus:ring-primary-500
                                focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600
                                dark:text-white
                                dark:focus:ring-primary-500 dark:focus:border-primary-500">
                ...
                <td class="py-3 px-6 ">
                    @if ($rab_id)
                    <selectwire:model.live.debounce.500ms="newBarangId" @disabled($isSeribu && !$newRabId && $withRab)
                ...
                <td class="py-3 px-6">
                    <div class="flex items-center">
                        <input type="number" wire:model.live.debounce.500ms="newJumlah" min="1" max="{{ $newMerkMax }}"
                            @disabled(!$newMerkId)
                ...
                <td class="px-6 py-3 {{ $isSeribu && $withRab ? '' : 'hidden' }}">
                    <textarea id="jumlah" wire:model.live.debounce.500ms="newKeterangan" rows="2"
    ...
    <div class="flex justify-center">
        ...
        @if ($isDataChanged)
        <button wire:click="updateData" {{-- Panggil method baru untuk update --}} wire:loading.attr="disabled"
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2 text-center">
                            <input type="number" min="0" max="{{ $jumlahTersedia }}"
                                wire:model.live.debounce.500ms="alokasiInput.{{ $lokasiKey }}"
