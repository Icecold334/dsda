                        ...
                        @if ($category === 'Riwayat')
                            ...
                            @foreach ($actions as $action)
                                <div class="flex items-center">
                                    <input type="radio" id="{{ $action }}" wire:model.live="selectedRiwayat"
                                        value="{{ $action }}"
                                        class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500">
                                    <label for="{{ $action }}"
                                        class="ml-2 text-sm text-gray-700">{{ $permissionLabels[$action] ?? $action }}</label>
                                </div>
                            @endforeach
                        @else
                            ...
                            @foreach ($actions as $action)
                                <div class="flex items-center">
                                    <input type="checkbox" id="{{ $action }}"
                                        wire:model.live="selectedPermissions" value="{{ $action }}"
